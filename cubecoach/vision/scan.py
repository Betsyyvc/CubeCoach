    ...
    try:
        while len(faces) < 6:
            ...
            if not ret:
                break
            display = frame.copy()
            cv2.putText(display, f"Scan faces: collected {len(faces)}/6. Press one of U,R,F,D,L,B when pointing at that face.", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
            ...
            if ch.lower() in FACE_KEYS:
                ...
                print(f"Captured face {face_label}")
    finally:
        cap.release()
